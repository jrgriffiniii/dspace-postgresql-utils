...
class Dataspace < Thor
  ...
  def student_theses_migrate
    ...
    prev_dspace.connection.select_items(class_year) do |rows|
      rows.each do |row|
        ...
        prev_dspace.connection.select_community_collections_query(item_id) do |collection_row|

          if !persisted_communities.key?(community_id)
            community_id = collection_row.values_at('community_id').first
            next_dspace.connection.update_community(next_item_id, item_id)
            logger.info "Updated communities..."

            persisted_communities[community_id] = community_id
          end

          next_dspace.connection.update_collection(next_item_id, item_id)
        ...
        logger.info "Updated authorization policies..."

        persisted_bundles = {}
        prev_dspace.connection.select_bundle_bitstreams(item_id) do |bitstream_row|
          ...
          if persisted_bundles.key?(bundle_id)
            ...
            bundle_values += row.values_at('primary_bitstream_id')
            next_dspace.connection.insert_bundle(next_item_id, *bundle_values)
            logger.info "Created bundle #{new_bundle_id}..."
          ...
          bitstream_rows = [new_bitstream_id]
          ...
          # new_bitstream_id = next_dspace.connection.insert_bitstream(new_bundle_id, bitstream_order, *bitstream_values)
          next_dspace.connection.insert_bitstream(new_bitstream_id, new_bundle_id, bitstream_order, *bitstream_values)
          logger.info "Created bitstream #{new_bitsteam_id}..."
